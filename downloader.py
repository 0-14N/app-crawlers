...
class Downloader:
    ...
    def open(self, url):
        ...
        while True:
            if fails > 3:
                print "skip %s" % self.url
                global glb_sqlite_file
                conn = sqlite3.connect(glb_sqlite_file)
                c = conn.cursor()
                c.execute('update apps set downloaded = -2 where url = ?', (self.url,))
                conn.commit()
                conn.close()
                break
            try:
                self.opening = urllib2.urlopen(self.url, timeout=20)
            except:
                print "HTTPError" 
                fails += 1
                continue
...
def get_undownloaded_url(sqlite_file, limit=50):
    print sqlite_file
    global glb_sqlite_file
    glb_sqlite_file = sqlite_file
...
def down_sqlite(argv):
    repo = argv[1]
    sqlite_file = argv[2]
    urls = get_undownloaded_url(sqlite_file, limit=2000)
    queue = Queue.Queue()
    for url in urls:
        queue.put(url)
    for i in range(2):
        t = DownloadThread(queue, repo, sqlite_file, None)
        t.daemon = True
        t.start()
    #while True:
    #    time.sleep(5)
    ...
    def run(self):
        ...
        while True:
            ...
            if downloader.download():
                ...
                c.execute('update apps set downloaded = 0 where url = ?', (url,))
                conn.commit()
